document.addEventListener("DOMContentLoaded", function(event){
  ...
  function ajax(){
    ...
    var id = p.options[p.selectedIndex].value;// il faut recuperer la valeur de l'input
    // console.info(personne);// permet d'afficher le nom de la personne dans la console

    var parameters = "id="+id;// il faut affecte une valeur afin de l'inserer dans la bdd prepare les parametre  à envoyer à la requete sql
    r.open("POST", "ajax.php", true);// je prepare le fichier php
    ...
    //     console.info(obj);
        document.getElementById("resultat").innerHTML = "<span style='background: #22d3a7'>employé"+p.value+" ajouté</span>";// je vais allez pointer sur ma div avec l'id resultat innerhtml convertir en html et une div avec du texte resultat qui permettra d'indiquer que le prenom a bien ete inserer
    //   }
    // }
